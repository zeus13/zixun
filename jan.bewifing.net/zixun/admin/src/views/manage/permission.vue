    ...
    <el-form ref="form" :model="user" label-width="100px" style="padding:30px 50px 30px 0;">
      ...
      <el-form-item label="权限" >
        ...
        <el-checkbox-group v-model="checkedPermissions" @change="handleCheckedCitiesChange">
          <el-checkbox v-for="(item, index) in permissions" :label="item" :key="index">{{item}}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>

      <el-form-item>
        <el-button type="primary" @click="updatePermit">修改</el-button>
      </el-form-item>
    </el-form>
...
<script>
import { getPermissionList, updatePermit, getUserDetail } from '../../api/getData';
...
export default {
  ...
  methods: {
    ...
    updatePermit () { // 更新权限
      ...
      self.user.role = role
      updatePermit(self.user).then(response => {
