    ...
    <div class="weui_cells weui_cells_access vux-search_show" id="search_show" v-show="isFixed && results.length && value">
      <div class="weui_cell" v-for="item in results" @click="handleResultClick(item)">
        <div class="weui_cell_bd weui_cell_primary">
          ...
          <a class="weui_media_box weui_media_appmsg bg-white pad-0"> 
            <div class="weui_media_hd"> 
              <img class="weui_media_appmsg_thumb" :src="!isEmpty(item.avatar) ? item.avatar+'?imageView2/1/w/120/h/120' : 'http://eyicdn.bewifing.net/doctor/doctor.png?imageView2/1/w/120/h/120'"> 
            </div> 
            <div class="weui_media_bd"> 
              ...
              <p class="weui_media_desc">
                {{item.sex}} {{ages(item.birth_time)}}岁<br>
                {{!isEmpty(item.hospital) ? item.hospital.name : ''}}{{!isEmpty(item.department) ? '【'+item.department.name+'】 ' : ''}}{{item.job}}
              </p> 
            </div> 
          </a>
...
<script>
import { isEmpty, getStamp, ages } from '../filters'
export default {
  props: {
    placeholder: {
      type: String,
  ...
  methods: {
    ...
    handleResultClick (item) {
      this.$emit('result-click', item.id)
/*      this.isCancel = true
      this.isFixed = false*/
    },
    touch () {
