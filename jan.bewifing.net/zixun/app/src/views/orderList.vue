...
<!--       <div class="tab">
      ...
      <tab bar-active-color="#d2bf97" defaultColor="#666" :line-width="3" active-color="#d2bf97">
        <tab-item @on-item-click="onItemClick(1)" :selected="status === 1">待付款</tab-item>
        <tab-item @on-item-click="onItemClick(2)" :selected="status === 2">待服务</tab-item>
        <tab-item @on-item-click="onItemClick(3)" :selected="status === 3">已完成</tab-item>
        <tab-item @on-item-click="onItemClick(4)" :selected="status === 0">全部订单</tab-item>
      </tab>
      <div class="orderList-box" v-for="(item,index) in orderList" :key="index">
        ...
        <div class="bd">
          <img class="bd-img" :src="item.goods_img + '?imageView2/1/w/200/h/200'">
          <div class="bd-cont">
        ...
        <div class="row">
          ...
          <span class="btn-check ml-10" @click="evaluate(item.goods_id, item.goods_name, item.goods_img)" v-show="getStatus(item) === 3">评价服务</span> 
          <div class="cb"></div>
        </div>
      </div>

      <div class="no-data" v-if="isEmpty(orderList)">还没有相关数据，<a href="javascript:void(0)" @click="goHome"> 点击查看更多产品信息...</a></div>

      <div class="evaluate-box" v-show="showEvaluate">
        <div class="evaluate-title">
          <div @click="closeEvaluate" class="div">
            <div class="back"></div>
          </div>
          评价
          <span class="btn-sumbit" @click="sumbitEvaluate">发布</span>
        </div>
        <div class="row-evaluate">
          <div class="div-img" :style="'background-image: url('+evaluateImg+'?imageView2/1/w/200/h/200);'"></div>
          <p class="evaluate-txt"> {{evaluateTitle}}</p>
        </div>
        <div class="textarea-box">
          <textarea placeholder="将你的对于本次服务的感觉分享给其他人吧" v-model="evaluateCont" class="textarea"></textarea>
        </div>
          <div class="pic-box-2">
            <div class="pic-box-item" v-for="(item1, index) in imageArrTemp" :style="'background-image: url('+item1+');'"></div>
            <div class="photo-box" @click="chooseImg">
              <img src="../assets/img/aimaspa/photo.png" class="photo-img">
              <div>添加图片</div>
            </div>
        </div>
      </div>
      <div v-for="a in mediaId">{{a}}</div>
    </div>
...
import { getOrderList, takeOverGoods, getJssdk, addEvaluateInfo } from '../api/getData'
...
export default {
  ...
  data () {
    return {
      ...
      type: '', // 提交类型
      wx: [], // 微信sdk
      status: 1, // 0全部1待付款2待服务3已完成
      orderList: [], // 订单列表
      showEvaluate: false, // 显示评论遮罩
      goods_id: 0, // 评论产品ID
      evaluateImg: '', // 评论产品图片
      evaluateTitle: '', // 评论产品题目
      evaluateCont: '', // 评论内容
      // evaluateArr: [], // 上传图片数组
      // chooseImgArr: [] // 选择图片数组
      imageArr: [],
  ...
  methods: {
    ...
    //             success: function (res) {
    //               urlArr.push(res.localData)
    //             }
    //           })
    //         } else {
    //           urlArr.push(localIds[i])
    //         }
    //       }
    //       self.chooseImgArr = urlArr
    //       self.updataImgFun(localIds)
    //     }
    //   })
    // },
    // updataImgFun (localIds) { // 上传图片
    //   let self = this
    //   var arrLen = localIds.length
    //   if (parseInt(arrLen) === 0) {
    //     return
    //   }
    ...
    //     success: function (res) {
    //       self.evaluateArr.push(res.serverId)
    //       self.updataImgFun(localIds)
    //     }
    //   })
    // },
    ...
    sumbitEvaluate () { // 提交信息
      var self = this
      if (isEmpty(self.evaluateCont)) {
        self.openPopupFun('alert', '评论不能为空！', 2)
        return
      }
      var object = {
        goods_id: self.goods_id,
        imgArr: self.mediaId,
        content: self.evaluateCont
      }
      self.showLoadingText = '提交中...'
      self.showLoading = true
      addEvaluateInfo(object).then(response => {
        var data = response.data
        console.log(data)
        if (data.code === 200) {
    ...
    evaluate (val1, val2, val3) {
      this.goods_id = val1
      this.evaluateTitle = val2
      this.evaluateImg = val3
