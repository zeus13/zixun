...
var picgame = {
    ...
    init: function(index, type) {
        ...
        for (var i = 0; i < img.data.length; i++) { // 不同点设置方法
            var item = img.data[i];
...
/*                $("#maskleft" + id).addClass("showborder"); // 显示红框*/
/*                $("#maskright" + id).addClass("showborder");*/
                self.action.clickData[id].find = 0; // 0未找到，1为找到
                self.action.clickNum = 0;//self.action.clickNum + 1;
                self.showTips();
                e.stopPropagation();
                e.preventDefault()
            });
    ...
    showTips: function() {
        ...
        this.action.correctPecent = 100;//parseInt(correctNum * 100 / this.action.clickNum) ? parseInt(correctNum * 100 / this.action.clickNum) : 0;
    ...
    prepare:function(){
        var t = this;
            ...
            $('.title').css({ width: "100px", margin: "0 0" });
            $('.title img').css({ margin: "20px 10px"});
            $('.btn').hide();
            $('.actlogo').hide();
            $('.returnindex').hide();
            $('.tips2').hide();
            ...
            $('.piccell').show();
            $('#timeshow').html("还剩 "+(t.obtime)+" 秒");// 设置观察初始显示时间
            $('.piccell2').hide();
            $('.tips1').show();
            $('.tips2').hide();
            $('#section3').hide();

            $('.font_size_min').html('记住纯悦的样子!');
            ...
            this.timer = setInterval(function() {
                ...
                $("#timeshow").html("还剩 "+(t.obtime - t.second)+" 秒"); // 观察剩余时间
                if (t.obtime - t.second <= 0 ) {
                    t.stopTimer();
                    $('.tips1').hide();
                    t.readyMsg('<div>记清楚了吗。我们开始了哦~</div>')
                }
            }, 1000)
    ...
    gamestart: function() {
        ...
        $('.piccell').hide();
        $('.piccell2').show();
        $('#timeshow2').html("还剩 "+(t.pltime)+" 秒");// 设置找茬初始显示时间
        $('.tips1').hide();
        $('.tips2').show();
        $('.tips-2').hide();
        $('.font_size_min').html('寻找不同!');
        ...
        this.init(this.currIndex);
        t.levelNum = t.levelNum + 1;
        this.timer = setInterval(function() {
            t.second = t.second + 1;

            $("#timeshow2").html("还剩 "+(t.maxSecond - t.second)+" 秒"); // 观察剩余时间
            if (t.maxSecond - t.second <= 0 ) {
                t.stopTimer();
                t.gameover('<div>闯过'+(t.levelNum-1)+'关！挑战失败！</div>')
    ...
    gameNext: function() {
        ...
        $.ajax({
            type: "POST",
            url: "/ci/pureyue/updateQuickspotScore",
            dataType: "json",
            success: function(msg){}
        });
        if (picimgs[picnum].length - 1 == this.currIndex) { // 通关
            $('.content').hide();
            $('#section3').show();            
            $('.returnindex').show();
            $('#notice').html('<div>恭喜你!共获得了15积分！</br>你的记忆非常不错,</br>记得按时补水,</br>让脑力和体力都处于巅峰状态!');
        } else { // 下一关提示设置
            title = "\u606d\u559c\u8fc7\u7b2c" + (self.currIndex + 1) + "\u5173";
            msg = "耗时" + (self.second) + "秒";
            dialog.alert({
                title: title,
                content: msg,
                buttons: {
